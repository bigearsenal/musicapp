...
class Nhaccuatui extends Module
	...
	createTables : ->
		...
		songsQuery = "CREATE TABLE IF NOT EXISTS " + @table.Songs + " (
					...
					songid int,
					song_key varchar(100),
					song_name varchar(255),
					song_artists varchar(150),
					artists varchar(150),
					topic varchar(100),
					genre varchar(100),
					albumid int,
					album_key varchar(100)
					);"
	...
	updateSongLinkKey : ()->
		...
		console.log "Running on: #{new Date(Date.now())}"
		console.log " |"+"Fetching albums'data  to table: #{@table.Songs}".magenta
		_q = "Select song_key from #{@table.Songs}exit"
		@connection.query _q, (err, songs)=>
			if err then console.log "CANNT get songs from database"
			else
				@stats.totalItems = songs.length
	...
	updateLyrics : ->
		...
		_q = "Select songid from #{@table.Songs} order by songid DESC"
		@connection.query _q, (err, songs)=>
			if err then console.log "CANNT get songs Songs from database"
			else
				# console.log Songs
				@stats.totalItems = songs.length
				console.log "Updating lyrics. N:"+@stats.totalItems
				@stats.currentTable = @table.Songs
				for song in songs
