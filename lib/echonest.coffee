...
class EchoNest extends Site
	...
	updateSongs : ->
		...
		@getFileByHTTP = (link, onSucess, onFail, options) ->
			...
			http.get config, (res) =>
				...
				.on 'error', (e) =>
					onFail  "Cannot get file from server. ERROR: " + e.message, option
	...
	testWithProxy : ->
		http = require 'http'
		getFileByHTTP = (link,options, callback) ->
			# proxy = options.proxies[Math.random()*options.proxies.length|0]
			# options.selectProxy = proxy
			config = 
				host: options.proxy.replace(/:[0-9]+/,'')
				port: parseInt options.proxy.replace(/.+:/,''),10
				path: link
			options.currentTime = Date.now()
			http.get config, (res) =>
					res.setEncoding 'utf8'
					data = ''
					# onSucess res.headers.location
					...
					else callback("The link is temporary moved: #{res.statusCode} #{JSON.stringify res.headers}",null,options)
				.on 'error', (e) =>
					callback  "Cannot get file from server. ERROR: " + e.message, null,options
		href = "http://www.nhaccuatui.com/playlist/am-duong-xa-cach-single-2013-luong-manh-hieu.j1f72LxnWYRp.html"
		#118.69.168.7:3128"
		#210.211.124.250:3128
		#123.30.129.54:8088
		proxies = []
		proxies.push "118.69.168.7:3128"
		proxies.push "210.211.124.250:3128"
		proxies.push "123.30.129.54:8088"
		proxies.push "109.207.50.182:8090"
		proxies.push "195.242.161.10:3128"
		proxies.push "94.142.27.4:3128"
		proxies.push "218.108.242.108:3128"
		proxies.push "202.116.1.149:8128"

		list = "69.197.132.80:3128
	192.3.1.96:7808
	...
	218.108.242.108:3128"
		goodProxies = []
		for proxy in list.split('\t')
			proxies.push proxy if proxy
		console.log proxies
		count = 0
		...
		for proxy in proxies
			options = 
				proxy : proxy
			getFileByHTTP href, options, (err,data,options)->
				count +=1
				if err then console.log err
				else 
					if data.search(/Âm Dương Xa Cách/g) > -1
						console.log "PROXY: #{options.proxy} WORKING: DURATION: #{(Date.now() - options.currentTime)/1000}s"
					else
						console.log "PROXY: #{options.proxy} NOT WORKING: DURATION: #{(Date.now() - options.currentTime)/1000}s".red

				if (Date.now() - options.currentTime)/1000 < 3
					goodProxies.push options.proxy
