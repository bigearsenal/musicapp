class Pace
	...
	op : (count, extraInfo,table) ->
		...
		if table is "kealbums" then  console.log "xxxxx"
		...
		if table is "kealbums" then  console.log "YYYYYYY"
		@outputProgress(table)
		
		if table is "kealbums" then  console.log "ZZZZZZZZZ"
		
		# The task is complete.
		# @finished()  if @current >= @total
		
		# Record end time.
		@time_end = new Date().getTime()
		@inner_time = @time_end - @time_start

		if table is "kealbums"
			# console.log "----------"
	...
	clear : ->
		@charm.erase("line").up(1).erase("line").up(1).erase("line").left 100


	outputProgress : (check)->
		...
		@charm.display('reset').left(200).down(1)
		# if check is "kealbums" then console.log "HELLO THIS IS OUTPUT"
		@outputStats(check)

	outputStats : (check)->

		# if @show_burden
		# 	@charm.write("    ").display("bright").write("Burden: ").display "reset"
		# 	@charm.write @time_burden.toFixed(2) + "% / " + @skip_steps
		# if @extraInfo
		@charm.write("Extra: ")
		@charm.write @extraInfo
		# if check is "kealbums"
		# 	console.log "THIS OUT STATS"
		@charm.left(200).down(1)
		@outputTimes(check)
