...
describe "#{name}",->
	...
	it "should have max songid in the range from max id in log file to itself plus 10", (done)->
		...
		wp.getQueryMethod query, (err,results)->
			...
			expect(max+10).to.be.above(content.lastSongId)
			done()
	...
	it "should have max videoid indentical to max id in log file", (done)->
		query = "select max(id) as max from #{table.video}"
		wp.getQueryMethod query, (err,results)->
			expect(err).to.equal(null)
			max = parseInt(results[0].max,10)
			expect(max).to.equal(content.lastVideoId)
			done()